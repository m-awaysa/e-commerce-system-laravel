...
<div id="carouselIndicators" class="carousel slide " data-bs-ride="true">
  <div class="carousel-indicators">
    <button type="button" data-bs-target="#carouselIndicators" data-bs-slide-to="0" class="active carousel-btn" aria-current="true"></button>
    @for($x=1;$x < $product->photo->count(); $x++)
      <button type="button" data-bs-target="#carouselIndicators" data-bs-slide-to="{{$x}}" class=" carousel-btn"></button>
      @endfor
  </div>
  <div class="carousel-inner">
    ...
    <div class="carousel-item <?php if (!$thereIsAnActive) {
      ...
      <img class="d-block w-100 single-image " src="{{asset('images/productImages/'.$photo->image)}}" onerror="this.src='  <?php echo asset('images/default.png') ?>  ' " alt="No Image">
    </div>

    @empty
    <div class="carousel-item active">
      <img class="d-block w-100 single-image " src="{{asset('images/productImages/'.$product->image)}}" onerror="this.src='  <?php echo asset('images/default.png') ?>  ' " alt="No Image">
    </div>
    @endforelse
...
<div class="div-info">
  ...
  <div class="single-cart">
    <livewire:cart-product :product="$product" :buttonName="'Add to cart'" classes="single-cart-btn btn btn-dark text-warning">
  </div>
                    ...
                    <div class="modal-footer">
                        <form action="{{route('guest.order')}}" method="post">
                            ...
                            <div class="w-100 row ">
                                ...
                                <div>
                                    <input type="number" name="company_number" value="{{old('company_number')}}" required min="2" max="999999999">
                                ...
                                <div>
                                    <input type="number" name="amount" value="{{old('amount')}}" required min="2" max="100">
                                    @error('amount')
                                    <div class="text-danger">{{$message}}</div>
                                    @enderror
                                </div>
